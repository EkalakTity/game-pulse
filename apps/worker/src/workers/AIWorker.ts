...
export function createAIWorker(concurrency = 2) {
  ...
  return new Worker<ProcessAIJobData>(
    ...
    async (job) => {
      ...
      const result = await aiProcessor.processArticle(article.title, article.summary);
      ...
      console.log(`[AIWorker] Processed article ${articleId} — ${result.hashtags.length} hashtags`);
    },
    {
